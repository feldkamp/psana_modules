...
if os.path.exists(source_dir+runtag+"/"+options.fileTag+"_avg_xaca.h5"):
	print source_dir+runtag+"/"+options.fileTag+"_avg_xaca.h5"
	f = H.File(source_dir+runtag+"/"+options.fileTag+"_avg_xaca.h5","r")
	d = N.array(f['/data/data'])
	f.close()
else:
	print "Source file "+source_dir+runtag+"/"+options.fileTag+"_avg_xaca.h5 not found, aborting script."
	sys.exit(1)
...
class img_class (object):
	...
	def draw_img(self):
		...
		self.axes = P.imshow(self.inarr, origin = 'lower', vmin = -1, vmax = 1)
